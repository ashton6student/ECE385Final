...
//	xil_printf("write data: %d, addr: %d", val, reg);
//	BYTE temp = MAXreg_rd(reg);
//	xil_printf("read data: %d", temp);
	if(code != 0){
		xil_printf("Error DDDDD:");
	}
...
BYTE MAXreg_rd(BYTE reg) {
	...
	writeBuffer[0] = reg;
	writeBuffer[1] = 0;
	int code = XSpi_Transfer(&SpiInstance, writeBuffer, writeBuffer, 2);
	//read return code from SPI peripheral 
	//if return code != 0 print an error
	if(code != 0){
		xil_printf("Error LMAOOOOO!!!");
	}
//	xil_printf("code: %d, addr:, %d, 1st: %d, 2nd: %d", code, reg, readBuffer[0], readBuffer[1]);
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
	XSpi_SetSlaveSelect(&SpiInstance, 0);
	//return val
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	writeBuffer[0] = reg;
	readBuffer[0] = 0;
	for(int i = 1; i < nbytes + 1; i += 1){
		writeBuffer[i] = 0;
	}
	//read data[n] from SPI, where n goes from 0 to nbytes-1
	int code = XSpi_Transfer(&SpiInstance, writeBuffer, writeBuffer, (nbytes+1));
	if(code != 0){
		xil_printf("Error ROFL");
	}
	for(int i = 1; i < nbytes + 1; i += 1){
			data[i-1] = writeBuffer[i];
	}
	//read return code from SPI peripheral 
	//if return code != 0 print an error
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
